/*
	...
	Filename: 	SunriseClient_system_deviceMineUral.sqf
...
while {true} do {
	...
	if (isEngineOn _vehicle) exitWith {
        _vehicle animateSource ["Stages",0] && sleep 13;
	...
	waitUntil {
		...
		if (isEngineOn _vehicle) exitWith {
            _vehicle animateSource ["Stages",0] && sleep 13;
			titleText[localize "STR_NOTF_MiningStopped","PLAIN"]; true};
        if (speed _vehicle >= 1) exitWith {titleText[localize "STR_NOTF_MiningStopped","PLAIN"]; true};
	...
	if(local _vehicle) then {
		...
		[_vehicle,(fuel _vehicle)-0.015] remoteExecCall ["SunriseClient_system_setFuel",_vehicle];
	};
