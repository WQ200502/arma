/*
	...
	Filename: 	SunriseClient_system_vehStoreItem.sqf
...
if !([_conditions] call SunriseClient_system_conditionsCheck) exitWith {hint "Данный предмет нельзя положить в это хранилище"};
...
switch (typeOf life_trunk_vehicle) do { 
	...
	default {
		_totalWeight = [life_trunk_vehicle] call SunriseClient_system_vehicleWeight;
	}; 
};
...
if !([false,_ctrl,_num] call SunriseClient_system_handleInv) exitWith {hint "Couldn't remove the items from your inventory to put in the vehicle.";};

private _index = [_ctrl,_inv] call SunriseClient_system_index;
