/*
	...
	Filename: 	SunriseClient_system_skyDive.sqf
...
_cops = false;
{
	if ((_x getVariable ["copDuty",false]) && player distance _x < 1000) then { _cops = true; };
} forEach playableUnits;
if (_cops) exitWith {hint "Вы не можете заниматься скайдайвингом будучи в розыске и с полицейскими в радиусе 1 км"};

_price = 20000;
if (CASH < _price) exitWith {hint format ["У вас нет необходимой суммы ($%1)",[_price] call SunriseClient_system_numberText]};
