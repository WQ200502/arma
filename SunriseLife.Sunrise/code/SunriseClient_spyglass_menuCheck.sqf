/*
	...
	Filename: 	SunriseClient_spyglass_menuCheck.sqf
...
while {true} do {
	{
		...
		if (_detection) exitWith {
			[steamid,0,format["Menu Based Hack D-%1",_targetDisplay],format["MenuBasedHack_%1 - %2",_targetName,_targetDisplay]] remoteExecCall ["SunriseServer_system_rcon",RSERV];
		};
	} foreach _displays;
	...
	if (!isNull (findDisplay 148)) then {
		...
		if ((lbSize 104)-1 > 3) exitWith {
			closeDialog 0;
			[steamid,0,"Menu Based Hack D-148", "findDisplay 148"] remoteExecCall ["SunriseServer_system_rcon",RSERV];
		};
	};
	...
	if (!isNull _display) then {
		{
			if (_x && !isNull _display) exitWith {
				closeDialog 0;
				[steamid,0,"Menu Based Hack D-54","MenuBasedHack_RscDisplayInsertMarker"] remoteExecCall ["SunriseServer_system_rcon",RSERV];
			};
		} forEach [
	...
	if (!isNull _display) then {
...

		{
			if (_x && !isNull _display) exitWith {
				closeDialog 0;
				[steamid,0,"Menu Based Hack D-131","MenuBasedHack_RscDisplayConfigureAction"] remoteExecCall ["SunriseServer_system_rcon",RSERV];
			};
		} forEach [
	...
	if (!isNull _display) then {
...

		{
			if (_x && !isNull _display) exitWith {
				closeDialog 0;
				[steamid,0,"Menu Based Hack D-163","MenuBasedHack_RscDisplayControlSchemes"] remoteExecCall ["SunriseServer_system_rcon",RSERV];
			};
		} forEach [
	...
	if !(unitRecoilCoefficient player in [1.2,1.15,1.1,1]) then {
		[["cheat",[format["unitRecoilCoefficient %1",unitRecoilCoefficient player]],profileName,steamid],"customLog"] call SunriseClient_system_hcExec;
		[steamid,0,"Menu Based Recoil Hack ","No_recoil_hack"] remoteExecCall ["SunriseServer_system_rcon",RSERV];
	};
