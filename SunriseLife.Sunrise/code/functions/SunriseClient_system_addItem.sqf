/*
	...
	Filename: 	SunriseClient_system_addItem.sqf
		...
		switch (_type) do {
			case "Glasses": {
				if (EQUAL(PGOOGLES,"")) then {
					...
					[PGOOGLES] call SunriseClient_system_addItemToInventory; //попробуем добавить текущий предмет в инвентарь
			...
			case "Headgear": {
				if (EQUAL(PHEADGEAR,"")) then {
					...
					[PHEADGEAR] call SunriseClient_system_addItemToInventory; //попробуем добавить текущий предмет в инвентарь
			...
			case "Vest": {
				if (EQUAL(PVEST,"")) then {
					...
					[PVEST] call SunriseClient_system_addItemToInventory; //попробуем добавить текущий предмет в инвентарь
					private _items = PVEST_ITEMS;
					removeVest player;
					player addVest _item;
					{
						[_x,"vest"] call SunriseClient_system_addItemToInventory;
					} forEach _items;
					_return = true;
				};
			};
			case "Uniform": {
				if (EQUAL(PUNIFORM,"")) then {
					...
					[PUNIFORM] call SunriseClient_system_addItemToInventory; //попробуем добавить текущий предмет в инвентарь
					private _items = PUNIFORM_ITEMS;
					removeUniform player;
					player forceAddUniform _item;
					{
						[_x,"uniform"] call SunriseClient_system_addItemToInventory;
					} forEach _items;
					_return = true;
				};
			};
			case "Backpack": {
				if (EQUAL(PBACKPACK,"")) then {
					...
					clearAllItemsFromBackpack player;
					{
						[_x,"backpack"] call SunriseClient_system_addItemToInventory;
					} forEach _items;
					_return = true;
				};
			};
	...
	case "Item": {
		switch (true) do {
			...
			case (_type in ["AccessoryMuzzle","AccessoryPointer","AccessorySights","AccessoryBipod"]) : {
				_return = [_item] call SunriseClient_system_addItemToWeapon;
			};
			//карта, бинокль и т.д.
