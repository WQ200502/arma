/*
	...
	Filename: 	SunriseClient_system_playerTags.sqf
	...
	if (!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {GVAR_RNAME(_x) != ""}) then {
		if (NOTINVEH(_x)) then {
			...
			if ((_x distance player) < 15 && {count _sPos > 0}) then {
				...
				if (_x getVariable ["newbie",false]) then {
					drawIcon3D ["\sunrise_config\kepka.paa", [1,1,1,1], (visiblePosition _x select [0,2]) + [((_x modelToWorld (_x selectionPosition "head")) select 2) + 0.85], 1.5, 1.5, 0, "", 1, 0.05];
				};
				if (_x in (units grpPlayer) OR !(EQUAL(_indexInIds,-1))) then {
					...
					if (!isNil {(group _x) GVAR "gang_name"}) then {
						if !(group _x isEqualTo life_copGroup) then {
