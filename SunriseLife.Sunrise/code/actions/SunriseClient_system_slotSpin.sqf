/*
	...
	Filename: 	SunriseClient_system_slotSpin.sqf
...
["cash","take",_betamt] call SunriseClient_system_myCash;
...
while{_a =_a + 1; _a < 15} do {
	...
	uiSleep .03;
	_slotPic1 ctrlSetText format["\sunrise_pack\pictures\slot\slot_%1.paa",_number];
	_number = ceil(random 7);
	uiSleep .03;
	_slotPic2 ctrlSetText format["\sunrise_pack\pictures\slot\slot_%1.paa",_number];
	_number = ceil(random 7);
	uiSleep .03;
	_slotPic3 ctrlSetText format["\sunrise_pack\pictures\slot\slot_%1.paa",_number];
};
...
if (EQUAL(_winnings,0)) then {
	if (EQUAL(_slot1,_slot2) OR EQUAL(_slot2,_slot3)) then {
		_winnings = _betamt * 0.5;
	};
};

if (_winnings > 0) then {
	player say3D "win";
	["cash","add",_winnings,"slotSpin"] call SunriseClient_system_myCash;	
	hint parseText format[localize "STR_NOTF_SlotWin",[_winnings] call SunriseClient_system_numberText];		
};
