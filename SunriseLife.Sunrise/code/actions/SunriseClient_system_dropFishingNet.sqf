/*
	...
	Filename: 	SunriseClient_system_dropFishingNet.sqf
...
{	
	...
	if ([true,_type,_val] call SunriseClient_system_handleInv) then {
		deleteVehicle _x;
		titleText[format[(localize "STR_NOTF_Fishing"),_typeName],"PLAIN"];
	};

	if ((call SunriseClient_system_isInterrupted) OR (NOTINVEH(player)) OR (isEngineOn _boat) OR (EQUAL((driver _boat),player)) OR (life_carryWeight >= life_maxWeight)) exitWith {};
