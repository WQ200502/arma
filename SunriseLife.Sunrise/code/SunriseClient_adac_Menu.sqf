/*
	...
	Filename: 	SunriseClient_adac_Menu.sqf
...
if (isNull (findDisplay 3700)) then {
	createDialog "SunriseDialogAdac";
...
if (EQUAL(_maxBoxes,0)) then {
	...
	_boxesAddButton buttonSetAction "[adac_vehicleToUpgrade,""boxes"",""add""] call SunriseClient_adac_Setup";
	_boxesRemoveButton buttonSetAction "[adac_vehicleToUpgrade,""boxes"",""del""] call SunriseClient_adac_Setup";
...
if (EQUAL(_canNitro,1)) then {
	...
	if (!isNil {_vehicle GVAR "adacNitroUsed"}) then {
		_nitrobutton buttonSetAction "[adac_vehicleToUpgrade,""nitro"",""del""] call SunriseClient_adac_Setup";
		_nitrobutton ctrlSetText localize "STR_ADAC_NitroRemove";
		_nitroInfo = localize "STR_ADAC_NitroAlreadySet" + "<br/>";
	} else {
		_nitrobutton buttonSetAction "[adac_vehicleToUpgrade,""nitro"",""add""] call SunriseClient_adac_Setup";
