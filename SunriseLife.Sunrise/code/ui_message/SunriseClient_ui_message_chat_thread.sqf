...
if (ui_message_chat_array isEqualTo []) exitWith {};
	...
	if (diag_tickTime >= _statusChangeAt) then 
	{
		switch (_status) do 
		{
			case 0: 
			{
				...
				ui_message_chat_array set [_forEachIndex, [_toastControl, 1, diag_tickTime + 1]];
			};
			case 1:
			{
				ctrlDelete _toastControl;
				ui_message_chat_array deleteAt _forEachIndex;
			};
		};
	};
}
