...
function CreatePlayer(source, permission_level, money, bank, identifier, license, group, roles)
	...
	rTable.setMoney = function(m)
		if type(m) == "number" then
			...
			log('ES_ERROR: 设置货币时似乎有问题，然后输入了一个数字.')
			print('ES_ERROR: 设置货币时似乎有问题，然后输入了一个数字.')
		end
	end
	...
	rTable.setBankBalance = function(m)
		if type(m) == "number" then
			...
			log('ES_ERROR: 设置银行时似乎有问题，然后输入了一个数字.')
			print('ES_ERROR: 设置银行时似乎有问题，然后输入了一个数字.')
		end
	end
	...
	rTable.addMoney = function(m)
		if type(m) == "number" then
			...
			log('ES_ERROR: 加钱的时候好像有个问题，要加的是另外一种类型的号码.')
			print('ES_ERROR: 加钱的时候好像有个问题，要加的是另外一种类型的号码.')
		end
	end
	...
	rTable.removeMoney = function(m)
		if type(m) == "number" then
			...
			log('ES_ERROR: 移钱时似乎有问题，另一种类型的号码试图被移走.')
			print('ES_ERROR: 移钱时似乎有问题，另一种类型的号码试图被移走.')
		end
	end
	...
	rTable.addBank = function(m)
		if type(m) == "number" then
			...
			log('ES_ERROR: 添加到银行时似乎有问题，试图添加另一种类型的号码.')
			print('ES_ERROR: 添加到银行时似乎有问题，试图添加另一种类型的号码.')
		end
	end
	...
	rTable.removeBank = function(m)
		if type(m) == "number" then
			...
			log('ES_ERROR: 从银行中删除时似乎有问题，尝试删除的是另一种类型的号码.')
			print('ES_ERROR: 从银行中删除时似乎有问题，尝试删除的是另一种类型的号码.')
		end
	end
	...
	rTable.displayMoney = function(m)
		if type(m) == "number" then
			...
			log('ES_ERROR: 显示货币时似乎有问题，试图显示不同类型的数字.')
			print('ES_ERROR: 显示货币时似乎有问题，试图显示不同类型的数字.')
		end
	end
	...
	rTable.displayBank = function(m)
		if type(m) == "number" then
			...
			log('ES_ERROR: 显示银行时似乎有问题，试图显示另一种类型的号码.')
			print('ES_ERROR: 显示银行时似乎有问题，试图显示另一种类型的号码.')
		end
	end
	...
	rTable.setPermissions = function(p)
		if type(p) == "number" then
			...
			log('ES_ERROR: 设置权限时出现问题，设置了不同的类型和号码.')
			print('ES_ERROR: 设置权限时出现问题，设置了不同的类型和号码.')
		end
	end
	...
	rTable.giveRole = function(role)
		for k,v in pairs(self.roles)do
			if v == role then
				print("User (" .. GetPlayerName(source) .. ") 已经具有此角色")
				return
			end
		end
