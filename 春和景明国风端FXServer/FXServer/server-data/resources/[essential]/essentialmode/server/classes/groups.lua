...
AddEventHandler("es:addGroup", function(group, inherit, aceGroup)
	...
	if(type(group) ~= "string")then
		log('ES_ERROR: 创建新组时似乎有问题，请确保输入了正确的 "group" as "string"')
		print('ES_ERROR: 创建新组时似乎有问题，请确保输入了正确的 "group" as "string"')
	end

	if(type(inherit) ~= "string")then
		log('ES_ERROR: 创建新组时似乎有问题，请确保输入了正确的 "inherit" as "string"')
		print('ES_ERROR: 创建新组时似乎有问题，请确保输入了正确的 "inherit" as "string"')
	end
