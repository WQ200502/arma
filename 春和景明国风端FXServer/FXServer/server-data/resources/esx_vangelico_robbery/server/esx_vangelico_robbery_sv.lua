...
AddEventHandler('esx_vangelico_robbery:rob', function(robb)
	...
	if Stores[robb] then
		...
		if (os.time() - store.lastrobbed) < 600 and store.lastrobbed ~= 0 then
			...
			TriggerClientEvent('esx:showNotification', source, _U('already_robbed') .. (900 - (os.time() - store.lastrobbed)) .. _U('seconds'))
			return
		end
...
AddEventHandler('esx_vangelico_robbery:gioielli1', function()
	...
	xPlayer.addInventoryItem('jewels', math.random(5))
end)
