...
function OpenBoss()
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
	...
	local elements = {}
		...
		table.insert(elements, {label = '你有: $' .. data[1].money .. ' 在你的公司',    value = ''})
		table.insert(elements, {label = '出货',    value = 'shipments'})
        table.insert(elements, {label = '放入物品出售', value = 'putitem'})
        table.insert(elements, {label = '拿出一项出售',    value = 'takeitem'})
        table.insert(elements, {label = '在您的公司中投入资金',    value = 'putmoney'})
        table.insert(elements, {label = '从贵公司拿钱',    value = 'takemoney'})
        table.insert(elements, {label = '贵公司更改名字: $' .. Config.ChangeNamePrice,    value = 'changename'})
		table.insert(elements, {label = '卖掉你的垃圾公司 $' .. math.floor(data[1].ShopValue / Config.SellValue),   value = 'sell'})
...
function OpenShipments(id)
	...
	table.insert(elements, {label = '订购产品', value = 'buy'})
	table.insert(elements, {label = '出货量', value = 'shipments'})
...
function GetAllShipments(id)
	...
	local once2 = true
		...
		for i=1, #items, 1 do
			...
			table.insert(elements, {label = '--准备好发货--'})
			table.insert(elements, {label = '获取所有货件', value = 'pickup'})
			once2 = false
			end
			...
			if result.cb ~= nil then
                ...
                TriggerServerEvent('esx_kr_shops-robbery:UpdateCanRob', id)
                    ...
                    local coords = {
						...
						TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~r~你的商店 ~b~(" .. result.name .. ')~r~ 正在抢劫', id)
